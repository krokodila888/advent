...
// Define element types: 'door' or 'window'
const elementTypes = [
  'door', 'window', 'window', 'door', 'window', // 1-5
  'door', 'window', 'window', 'door', 'window', // 6-10
  'door', 'window', 'window', 'door', 'window', // 11-15
  'window', 'door', 'window', 'door', 'window', // 16-20
  'door', 'window', 'window', 'door', 'window'  // 21-25
];

// Window sizes
const windowSizes = [
  'window-small', 'window-medium', 'window-large', 'window-small', 'window-medium',
  'window-large', 'window-small', 'window-medium', 'window-large', 'window-small',
  'window-medium', 'window-large', 'window-small', 'window-medium', 'window-large',
  'window-small', 'window-medium', 'window-large', 'window-small', 'window-medium',
  'window-large', 'window-small', 'window-medium', 'window-large', 'window-medium'
];

// Window shapes
const windowShapes = [
  'window-rect', 'window-square', 'window-tall', 'window-rect', 'window-square',
  'window-tall', 'window-rect', 'window-square', 'window-tall', 'window-rect',
  'window-square', 'window-tall', 'window-rect', 'window-square', 'window-tall',
  'window-rect', 'window-square', 'window-tall', 'window-rect', 'window-square',
  'window-tall', 'window-rect', 'window-square', 'window-tall', 'window-square'
];
...
const doorDecorations = [
  'wreath', 'garland', 'none', 'wreath', 'bow', 
  'holly', 'garland', 'wreath', 'none', 'bow',
  'wreath', 'holly'
];

// Christmas decorations for windows
const windowDecorations = [
  'candle', 'lights', 'bow', 'none', 'candle',
  'lights', 'holly', 'candle', 'bow', 'none',
  'lights', 'candle', 'holly'
];
...
export function CalendarWindow({
  ...
  const glowClass = isActive && !isOpened 
    ? (isExactMatch ? 'window-glow-intense' : 'window-glow-subtle')
    : '';
  ...
  const renderWindowPanes = (panes: number) => {
    ...
    return (
      <div className="window-panes" style={{ 
  ...
  return (
    <div
      className={`calendar-window ${glowClass} ${
      ...
      {isDoor ? (
        ...
        <div className={`door-element ${doorDesign.type}`}>
          ...
          <div className={`door-frame frame-${doorDesign.frame}`}>
            <div className={`door-panel wood-${doorDesign.wood}`}>
              {doorDesign.type === 'door-double' && (
                <div className="door-split" />
              )}
              ...
              <div className="door-hardware">
                ...
                {doorDesign.hasKeyhole && (
                  <Key className="door-keyhole" size={10} />
                )}
              </div>
        ...
        // WINDOW ELEMENT
        <div className={`window-element ${windowShapes[windowNumber - 1]} ${windowSizes[windowNumber - 1]}`}>
          {renderWindowDecoration(windowDecoration)}
          <div className="window-frame">
            <div className={`window-glass glass-${windowDesign.glass}`}>
              {renderWindowPanes(windowDesign.panes)}
              {windowDesign.shape === 'arched' && <div className="window-arch" />}
              {isOwlWindow && (
                <Bird className="owl-icon" size={16} />
              )}
            </div>
            <span className="window-number gold">{windowNumber}</span>
          </div>
